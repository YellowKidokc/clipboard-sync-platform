...
export async function applyRules(clip: ClipInput, deviceName?: string): Promise<ClipInput> {
  ...
  return applyRulesToClip(activeRules, clip, deviceName);
}

export async function applyRulesToClip(ruleSet: Rule[], clip: ClipInput, deviceName?: string): Promise<ClipInput> {
...
export function matches(rule: Rule, clip: ClipInput): boolean {
  ...
  switch (rule.action) {
    ...
    case "route_folder": {
      ...
      const [folder] = await db
        .select()
        .from(folders)
        .where(and(eq(folders.userId, clip.userId), like(folders.pathTemplate, `${resolved}%`)))
        .limit(1);
